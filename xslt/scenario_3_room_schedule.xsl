...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:ns="https://www.datasciencetech.institute/scheduling">
    ...
    <xsl:template match="/ns:culturalCenter">
        <html>
            <body>
                <h1 style="color:#06730F">Schedule of Room <u>002</u></h1>
                <table border="1" bgcolor="#fffff">
                            ...
                            <xsl:for-each select="document('')/*/ns:days/day">
                                ...
                                <td>
                                    ...
                                    <xsl:if test="$activitiesCount = 2">
                                        <xsl:for-each select="/ns:culturalCenter/ns:timeslot[substring(ns:startTime, 1, 2) &lt;= substring($currentSlot, 1, 2) and substring(ns:endTime, 1, 2) &gt; substring($currentSlot, 1, 2) and ns:daysOfWeek = $currentDay and ns:roomID='002']">
                                            <div style="border:2px ; margin:5px; width: 70%; float: left; background-color:#ECFFD4">
                                                ...
                                                Room: <strong style="color:#960F41"><xsl:value-of select="ns:roomID" /></strong><br/>
                                                Teacher: <b style="color:#06730F"><xsl:value-of select="ns:teacher" /></b>
                                            </div>
                                        </xsl:for-each>
                                    </xsl:if>
